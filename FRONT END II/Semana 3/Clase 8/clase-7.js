...
function renderizarAlbumes(listado) {
    ...
    mostrarDatosEnPerfil(listado)
};

renderizarAlbumes(listadoAlbumes);
...
function mostrarDatosEnPerfil(listadoAlbumes) {
    // desarrollar la función 👇
    let cantidadAlbumes = listadoAlbumes.length
    let cantidadFavoritos = 0
    listadoAlbumes.forEach((album) => {
        if (album.like === true){
            cantidadFavoritos++;
        }
    })
    let cantAlbums = document.querySelector("#cant-albums")
    cantAlbums.innerHTML = `${cantidadAlbumes} ${cantidadAlbumes === 1 ? 'album' : 'álbumes'}`
    let cantFavoritos = document.querySelector("#cant-favoritos")
    cantFavoritos.innerHTML = `${cantidadFavoritos} ${cantidadFavoritos === 1 ? 'favorito' : 'favoritos'}`
}
