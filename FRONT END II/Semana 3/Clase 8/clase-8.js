...
function eliminarAlbum() {
    ...
    let albumesFamosos = listadoAlbumes
    document.addEventListener("keydown", event =>{
        if (event.key === 'f'){
            let albumEliminar = prompt("Ingrese el nombre del album a eliminar.").toLowerCase()
            let indice = albumesFamosos.findIndex((elemento) => {
                return elemento.nombre.toLowerCase().includes(albumEliminar)
            })
            albumesFamosos = albumesFamosos.filter((elemento, index) => {
                return index != indice
            })
            //👇 post click debemos renderizar nuevamente las tarjetas
