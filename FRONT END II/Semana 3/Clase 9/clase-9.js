...
window.addEventListener('load', function () {
    ...
    intervaloEspera = setTimeout(() => {
        let masMusica = confirm("¿Desea conocer más música?")
        if (masMusica){
            this.open("https://open.spotify.com/")
        } else{
            let perfil = document.querySelector(".perfil")
            console.log(perfil)
            perfil.innerHTML += `<p>Usuario oficial de Spotifront</p>`
            perfil.querySelectorAll("p")[1].setAttribute("style", "color:Green")
            perfil.addEventListener("click", () => {
                alert("Gracias por confiar en nosotros.")
                this.clearTimeout(intervaloEspera)
            })
        }
    }, 12000)
})
