...
function mostrarMensajeExito(listado) {
    ...
    let botonFormulario = document.querySelector("form button")
    if (listado.length === 0){
        const divTemplate = document.createElement('div')
        divTemplate.setAttribute('id', 'resultado_exitoso')
    ...
    setTimeout(() => {
        const cajaResultadoExitoso = document.querySelector('#resultado_exitoso');
        if (cajaResultadoExitoso) {
            cajaResultadoExitoso.remove()
        }
