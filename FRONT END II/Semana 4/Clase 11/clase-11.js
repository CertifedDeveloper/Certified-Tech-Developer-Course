...
function validarInformacion(usuario) {
    ...
    if(!Number.isNaN(Number(usuario.nombre)) || usuario.nombre.length < 3){
        errores.push("El nombre debe tener al menos 3 caracteres.")
    }
    if (usuario.password.split(" ").join("") < 6){
        errores.push("La contraseña debe tener al menos 6 caracteres, entre letras y símbolos.")
    }
    if(Number.isNaN(Number(usuario.telefono)) || usuario.telefono.length < 10){
