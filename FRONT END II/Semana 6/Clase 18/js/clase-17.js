...
function consultaApi(endpoint) {
    fetch(endpoint)
        ...
        .catch(error => {
            alert("No se pudo consultar a la API")
        })    
}
...
function renderizarElementos(listado) {
    ...
    const parteComentarios = document.querySelector(".mostrar")
    parteComentarios.remove()

    comentarios.innerHTML = listado.map((item, i) => {
        if (i < 10)
        {
