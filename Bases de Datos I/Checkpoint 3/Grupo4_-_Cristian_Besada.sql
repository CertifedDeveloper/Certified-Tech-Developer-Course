...
WHERE hab.numero = 25;
-- el resultado esta correcto pero no hacia falta left join
...
-- el resultado esta correcto pero no hacia falta left join, tambien recordar que todos los cambos que listan en el select deben figurar en el group by
...
-- ok - pero recordar que todos los cambos que listan en el select deben figurar en el group by
...
AND h.nombre = "Joel"
GROUP BY h.id;
-- ok - tener en cuenta que todos los campos que estan en el select deben estar en el group by
